// ./detect_aruco_image -i ../images/example_01.png -t DICT_5X5_100
// ./detect_aruco_image -i ../images/example_02.png -t DICT_ARUCO_ORIGINAL
...
void detect_aruco_image(const std::string &image_path, const std::string &type) {
...
int main(int argc, char **argv) {
  ...
  CLI::App app{"detect_aruco_image"};
  std::string image_path;
  std::string type;

  app.add_option("-i,--image", image_path, "path to the input image that has the aruco marker")
      ->required();
  app.add_option("-t,--type", type, "Type of aruco to detect")
      ->required();
  CLI11_PARSE(app, argc, argv);

  detect_aruco_image(image_path, type);
